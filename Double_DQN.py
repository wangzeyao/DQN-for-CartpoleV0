...
class DQNAgent:
    ...
    def replay(self, batch_size):
        ...
        for state, action, reward, next_state, done in minibatch:
            target = reward
            if not done:
                a_max = np.argmax(self.model.predict(next_state))  #先在当前Q网络中先找出最大Q值对应的动作
                # principal code for DQN,we update the Q-value by using the q-value formula
                target = (1 - ALPHA) * (self.model_t.predict(state)[0][action]) + ALPHA * (reward + GAMMA * self.model_t.predict(next_state)[0][a_max])  #用amax在目标网络里面去计算目标Q值
